...
describe("PositionRowMapper", () => {
  ...
  it("should map a row with a stored last price into a position carrying that price as a snapshot", () => {
    const row = { id: "world", userId: "user-1", name: "Fidelity MSCI World", ticker: "0P0001CLDK.F", type: "fondo", units: 30.12, groupName: "rv", lastPrice: 13.9762, updatedAt: 1751000000000 };
  ...
  it("should default the price to zero when the row has never received a price snapshot", () => {
    const row = { id: "new", userId: "user-1", name: "Nueva posición", ticker: "", type: "fondo", units: 0, groupName: "rv", lastPrice: null, updatedAt: 1751000000000 };
  ...
  it("should map a position back into a row carrying the owning user and the given updatedAt timestamp", () => {
    ...
    expect(row).toEqual({ id: "btc", userId: "user-1", name: "Bitcoin", ticker: "BTC-EUR", type: "cripto", units: 0.003441, groupName: "btc", lastPrice: 60848, updatedAt: 1751000000000 });
  });
});
