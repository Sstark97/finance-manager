...
function buildDependencies(overrides: Partial<LoadInitialAppStateDependencies> = {}): LoadInitialAppStateDependencies {
  ...
  const seedBudget: BudgetSnapshot = {
...
describe("LoadInitialAppState", () => {
    ...
    const dependencies = buildDependencies({
      loadGoalsSettings: {
        invoke: vi.fn().mockImplementation(async () => {
          if (!hasBeenSeeded) throw new Error("goals_settings singleton row is missing");
          return dependencies.seedGoalsSettings;
        }),
      },
  ...
  it("should not seed goals settings when another slice fails to seed, keeping goals_settings absent so the next load retries", async () => {
    const dependencies = buildDependencies({
      loadGoalsSettings: { invoke: vi.fn().mockRejectedValue(new Error("goals_settings singleton row is missing")) },
  ...
  it("should skip seeding goals settings when a concurrent request has already seeded them", async () => {
    ...
    const dependencies = buildDependencies({
      loadGoalsSettings: {
        invoke: vi.fn().mockImplementation(async () => {
          if (!hasBeenSeededByConcurrentRequest) throw new Error("goals_settings singleton row is missing");
          return dependencies.seedGoalsSettings;
        }),
      },
      savePortfolio: {
